(function(){
	...
	var plugin = function(){
			...
			'bindKeypress': function(){
				$lib(document).keyup(function (e) {
						...
						if(e.which == 13 && settings.isCtrl){
							...
							$lib.each(settings.storage.elementStorage, function(i, v){
		...
		AtKit.addButton(
			'findParagraphs', 
			'Find Paragraphs',
			AtKit.getPluginURL() + 'document-icon.png',
			function(dialogs, functions){
				settings.searchType = 'h1, h2, h3, h4, h5';
				var elements = AtKit.call('locateElements', $lib(settings.searchType));
		...
		AtKit.addButton(
			'findLinks',
			"Find Links",
			AtKit.getPluginURL() + 'document-export.png',
			function(dialogs, functions){
