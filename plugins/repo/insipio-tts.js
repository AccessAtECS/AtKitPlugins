	...
	var plugin = function(){
		...
		var TTSDialogs = {
			"options": {
				...
				"body": AtKit.localisation("tts_what") + " <br /><!--<button id=\"sbStartTTS\"> " + AtKit.localisation("tts_page") + "</button>--> <button id=\"sbStartTTSSelection\"> " + AtKit.localisation("tts_selected") + "</button>"
			},
			"starting": {
		...
		AtKit.addFn('sendTTSChunk', function(args){
			...
			var urlString = settings.speechServicesURL + 'insipio-tts/request.php?rt=tts&v=2&i=1&l=' + AtKit.getLanguage() + '&id=' + args.reqID + '&data=' + payload + "&chunkData=" + args.totalBlocks + "-" + args.block;
		...
		AtKit.addFn('countdownTTS', function(arg){
			if(isNaN(arg.timeLeft)){
				...
				if(arg.timeLeft == 0){
					...
					if($lib.browser != "msie"){
						$lib('body').append( $lib("<div id=\"flashContent\"><OBJECT classid=\"clsid:D27CDB6E-AE6D-11cf-96B8-444553540000\" codebase=\"http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,40,0\" width=\"1\" height=\"1\" id=\"audioe\"> <PARAM name=movie value=\"" + settings.baseURL + "resources/player/player-licensed.swf\"></PARAM> <PARAM name=flashvars value=\"file=" + settings.speechServicesURL + "cache/" + arg.id + ".xml&autostart=true&playlist=bottom&repeat=list&playerready=playerReady&id=" + audioContainer + "\"><PARAM name=allowscriptaccess value=\"always\" /><embed type=\"application/x-shockwave-flash\" pluginspage=\"http://www.macromedia.com/go/getflashplayer\" src=\"" + settings.baseURL + "resources/player/player-licensed.swf\" width=\"1\" height=\"1\" allowscriptaccess=\"always\" allowfullscreen=\"false\" flashvars=\"file=" + settings.speechServicesURL + "cache/" + arg.id + ".xml&autostart=true&playlist=bottom&repeat=list&playerready=playerReady\" name=\"audioe\" /> </OBJECT></div>") );
